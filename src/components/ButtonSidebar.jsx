...
function ButtonSidebar({ btnIcon, btnText, isOpen, m, onClick, title }) {

    const iconBox = { base: '40px', md: isOpen ? '40px' : '30px' }

    return (
        <Box >
            <Button title={title} w={'100%'} h={{ base: '100%', md: '50px' }} gap={2} maxW={isOpen ? '100%' : '45px'} borderRadius={{ base: 0, md: isOpen ? 0 : '2rem' }}
